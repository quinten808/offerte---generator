...
import { deleteQuote as deleteLocalQuote } from "@/app/lib/quote-storage";
import { deleteQuote as deleteSupabaseQuote } from "@/app/lib/supabase/quotes";
...
export default function OfferteDetailPage() {
  ...
  const localQuote = useQuotes().find((item) => item.id === id);
  const { quotes: supabaseQuotes, isLoading: isLoadingQuote, error: quoteError } = useSupabaseQuotes();
  const quote = supabaseQuotes.find((item) => item.id === id) ?? localQuote;
  const isLocalQuote = Boolean(localQuote) && !supabaseQuotes.some((item) => item.id === id);
  ...
  if (isLoadingQuote && !localQuote) return <p className="text-sm text-slate-500">Offerte laden...</p>;
  if (quoteError && !localQuote) return <section className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">{quoteError}</section>;
  ...
  async function remove() { if (!quote || !window.confirm(`Weet u zeker dat u ${quote.number} wilt verwijderen?`)) return; if (isLocalQuote) { if (deleteLocalQuote(quote.id)) router.push("/offertes"); return; } await deleteSupabaseQuote(quote.id); router.push("/offertes"); }
