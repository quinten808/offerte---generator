...
import { saveCompanySettings } from "@/app/lib/supabase/company-settings";
...
function SettingsForm({ initialSettings, onSave }: { initialSettings: CompanySettings; onSave: (settings: CompanySettings) => Promise<CompanySettings> }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const saved = await onSave({ ...settings, companyName: settings.companyName.trim(), email: settings.email.trim() });
      setSettings(saved);
      setSavedSettings(saved);
      setMessage("Instellingen zijn opgeslagen.");
    } catch (reason) {
  ...
  function uploadLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type)) {
      setLogoError("Kies een PNG-, JPG-, JPEG- of WEBP-bestand.");
      return;
    }
    if (file.size > 1024 * 1024) {
      setLogoError("Het logo mag maximaal 1 MB groot zijn.");
      return;
    }
    ...
    reader.onerror = () => setLogoError("Het logo kon niet worden gelezen.");
    reader.readAsDataURL(file);
  }

  return <form className="space-y-8" noValidate onSubmit={save}>
    {message && <p className={`rounded-lg border px-4 py-3 text-sm ${message === "Instellingen zijn opgeslagen." ? "border-green-200 bg-green-50 text-green-800" : "border-red-200 bg-red-50 text-red-800"}`} role="status">{message}</p>}
    ...
    <section><h2 className="text-lg font-semibold">Huisstijl</h2><div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-5"><p className="text-sm text-slate-600">Upload een PNG, JPG, JPEG of WEBP van maximaal 1 MB.</p><input accept="image/png,image/jpeg,image/webp" className="mt-4 block text-sm" onChange={uploadLogo} type="file" />{logoError && <p className="mt-2 text-sm text-red-700">{logoError}</p>}{settings.logoDataUrl && <div className="mt-5"><img alt="Logo-preview" className="max-h-32 max-w-xs rounded-lg border border-slate-200 bg-white p-2" src={settings.logoDataUrl} /><button className="mt-3 block text-sm font-medium text-red-700" onClick={() => update("logoDataUrl", null)} type="button">Logo verwijderen</button></div>}</div></section>
    {isDirty && <p className="text-sm text-amber-700">U heeft niet-opgeslagen wijzigingen.</p>}
    <button className="rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting ? "Opslaan..." : "Instellingen opslaan"}</button>
  </form>;
}
...
export default function InstellingenPage() {
  ...
  useEffect(() => {
    async function findLocalSettings() {
      await Promise.resolve();
      setLocalSettings(getLocalCompanySettingsForImport());
    }
  ...
  async function importLocalSettings() {
    ...
    try {
      ...
      removeLocalCompanySettingsAfterImport();
      setLocalSettings(undefined);
      await refresh();
    } catch (reason) {
      setImportError(reason instanceof Error ? reason.message : "Lokale instellingen importeren is niet gelukt.");
    } finally {
